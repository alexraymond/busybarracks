...
class Application(object):
	...
	def __init__(self):
		if not os.path.isdir(flags.log_directory):
			os.mkdir(flags.log_directory)
		self.train_logfile = flags.log_directory + '/train_results.log'
		# Training logger
		...
		self.thread_count = min(flags.environment_count,flags.groups_count) if flags.groups_count > 0 else 1
	...
	def create_session(self):
		...
		return tf.InteractiveSession(graph=tf.Graph(), config=config_proto)
	...
	def test(self):
		result_file = '{}/test_results_{}.log'.format(flags.log_directory, self.global_step)
		...
		stats_file = '{}/test_statistics.log'.format(flags.log_directory)
	...
	def save_checkpoint(self, global_step, saver, tf_session):
		...
		if flags.compute_plot_when_saving:
			plt.plot_files(log_files=[self.train_logfile], figure_file=flags.log_directory + '/train_plot.jpg')
		# Save Checkpoint
		print('Start saving..')
		saver.save(
			sess=tf_session, 
