...
class EnvironmentManager(object):
	...
	def __init__(self, model_size, group_id, environment_id=0, training=True):
		...
		if self._training:
			#logs
			if not os.path.isdir(flags.log_directory + "/performance"):
				os.mkdir(flags.log_directory + "/performance")
			if not os.path.isdir(flags.log_directory + "/episodes"):
				os.mkdir(flags.log_directory + "/episodes")
			formatter = logging.Formatter('%(asctime)s %(message)s')
			# reward logger
			self.__reward_logger = logging.getLogger('reward_{}_{}'.format(self.group_id, self.environment_id))
			hdlr = logging.FileHandler(flags.log_directory + '/performance/reward_{}_{}.log'.format(self.group_id, self.environment_id))
	...
	def log_episode_statistics(self, global_step):
		...
		if self.save_frame_info:
			...
			episode_directory = "{}/episodes/reward({}-{})_value_({})_step({})_thread({})".format(flags.log_directory, tot_reward, tot_manipulated_reward, avg_value, global_step, self.environment_id)
