...
# Environment
	...
	# tf.app.flags.DEFINE_string("env_type", "MultipleProteinAlignment", "Environment types: rogue, car_controller, sentipolc, MultipleProteinAlignment, or environments from https://gym.openai.com/envs")
	# tf.app.flags.DEFINE_string("env_type", "sentipolc", "environment types: rogue, car_controller, sentipolc, MultipleProteinAlignment, or environments from https://gym.openai.com/envs")
	# tf.app.flags.DEFINE_string("env_type", "rogue", "environment types: rogue, car_controller, sentipolc, MultipleProteinAlignment, or environments from https://gym.openai.com/envs")
	tf.app.flags.DEFINE_string("env_type", "BusyBarracks", "Environment types: BusyBarracks, rogue, car_controller, sentipolc, MultipleProteinAlignment, TextGenerator, or environments from https://gym.openai.com/envs")
...
# Intrinsic Rewards: Burda, Yuri, et al. "Exploration by Random Network Distillation." arXiv preprint arXiv:1810.12894 (2018).
	tf.app.flags.DEFINE_boolean("intrinsic_reward", False, "An intrinisc reward is given for exploring new states.")
	tf.app.flags.DEFINE_boolean("use_training_state", True, "Use intrinsic reward weights (the training state) as network input. Requires intrinsic_reward == True.")
...
# Experience Replay
	...
	tf.app.flags.DEFINE_float("replay_mean", 0, "Mean number of experience replays per batch. Lambda parameter of a Poisson distribution. When replay_mean is 0, then experience replay is not active.") # for A3C is 0, for ACER default is 4
	...
	tf.app.flags.DEFINE_boolean("runtime_advantage", True, "Whether to compute advantage at runtime, using always up to date state values instead of old ones.") # default True
...
# Reward manipulators
	tf.app.flags.DEFINE_string("extrinsic_reward_manipulator", 'lambda x: x', "Set to 'lambda x: x' for no manipulation. A lambda expression used to manipulate the extrinsic rewards.")
...
# Actor-Critic parameters
	...
	tf.app.flags.DEFINE_integer("groups_count", 4, "Number n of groups, the environments are divided equally in n groups. Usually we have a thread per group. Used to better parallelize the training.")
	tf.app.flags.DEFINE_integer("batch_size", 2**3, "Maximum batch size.") # default is 8
	# A big enough big_batch_size can significantly speed up the algorithm when training on GPU
	tf.app.flags.DEFINE_integer("big_batch_size", 2**6, "Number n > 0 of batches that compose a big-batch used for training. The bigger is n the more is the memory consumption.")
...
# Log
	...
	tf.app.flags.DEFINE_string("log_directory", "./log", "events directory")
	...
	tf.app.flags.DEFINE_string("show_episodes", 'none', "What type of episodes to save: random, best, all, none")
...
# Plot
	...
	tf.app.flags.DEFINE_integer("max_plot_size", 10, "Maximum number of points in the plot. The smaller it is, the less RAM is required. If the log file has more than max_plot_size points, then max_plot_size means of slices are used instead.")
