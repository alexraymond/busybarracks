...
class ExperienceBatch():
	...
	def get_cumulative_action(self, action, agents=None):
		action_values = getattr(self,action)
		# All agents
		if agents is None or len(agents) == self._model_size:
			return sum( sum(value) for value in action_values )
		# Not all agents
		return sum( sum(value) for agent,value in enumerate(action_values) if agent in agents )
